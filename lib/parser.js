'use strict';

var defaults = require('./config')
  , names = Object.keys(defaults.sections)
  , keys = Object.keys(null);

//
// Generate allowed config keys per section from the bitmasks.
//
names.forEach(function prepareKeys(section) {
  var mask = defaults.sections[section]
    , current;

  Object.keys(defaults.keys).forEach(function bitmask(bit) {
    current = keys[section] || [];
    if (mask & +bit) keys[section] = current.concat(defaults.keys[bit]);
  });
});

/**
 * Automatic parser interface for configuration changes.
 *
 * @constructor
 * @param {Configuration} config The parent configuration instance
 * @api private
 */
function Parser(config) {
  this.config = config;

  this.config.on('read', function read(type) {
    this[type]();
  }.bind(this));
}

/**
 * Parse files with a `.json` extension.
 *
 * @returns {Parser}
 */
Parser.prototype.json = function json() {
  this.config.data = JSON.parse(this.config.source);

  return this;
};

/**
 * Parses files with a `.cfg` extension.
 *
 * @returns {Parser}
 */
Parser.prototype.cfg = function cfg() {
  var data = this.config.source
    , section
    , current
    , name
    , hash
    , key;

  /**
   * Is config key the first part of the line?
   *
   * @param {String} line of content
   * @param {String} key in config
   * @return {Boolean}
   * @api private
   */
  function find(line, key) {
    return line.indexOf(key) === 0;
  }

  data.split(/\n/).forEach(function parse(line) {
    line = line.trim();

    // No content on line just return.
    if (!line.length) return;

    // Keep track of the section.
    section = names.filter(find.bind(this, line))[0];

    if (section) {
      name = line.split(' ')[1]; // Named section support.
      return current = section;
    }

    // Check content against known keys in current section.
    key = keys[current].filter(find.bind(this, line))[0];

    if (!key.length) return;

    // Store the value if we got a key match, and add the comment (if present).
    hash = line.split('#');
    this.config[current](name).add(key, hash[0].substr(key.length).trim())(hash[1]);
  }, this);

  return this;
};

//
// Expose the module.
//
module.exports = Parser;
